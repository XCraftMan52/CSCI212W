 ...
 public void SetPrice(double tPrice)
 {
	...
	public static Book ParsingOneBook(String tCsvBook)
	{
		...
		if (tCsvBook.length() > 0)
		{
			...
			String dateString = tCsvBook.substring(pos1,pos2).trim();
			LocalDate tDate = null;
			// Try different date formats
			DateTimeFormatter[] formatters = {
				DateTimeFormatter.ofPattern("M/d/yy"),      // 5/12/21
				DateTimeFormatter.ofPattern("M/d/yyyy"),    // 5/12/2021
				DateTimeFormatter.ofPattern("MM/dd/yyyy"),  // 05/12/2021
				DateTimeFormatter.ofPattern("yyyy-MM-dd")   // 2021-05-12
			};
			...
			for (DateTimeFormatter formatter : formatters)
			{
				try
				{
					tDate = LocalDate.parse(dateString, formatter);
